...
//                               float altitude, float thickness)
//            {
//                // Get UV coordinates for this direction
//                float2 uv = directionToCloudUV(dir, altitude, time, speed, scale);
//                
//                // Generate cloud pattern
//                float clouds = fbm(uv);
                
                // Add detail
//                float detail = fbm(uv * 2.0 + float2(time * 0.1, 0.0)) * 0.3;
//                clouds = clouds * 0.7 + detail * 0.3;
                
                // Apply vertical thickness - clouds should fade at top/bottom of their layer
//                float verticalPos = uv.y / scale;  // Get back to normalized vertical position
//                float heightFalloff = 1.0 - saturate(abs(verticalPos - altitude) / (thickness * 0.5));
...
//                clouds *= saturate(1.0 - horizon * 1.5);
